module ShopifyToolkit::Schema
  ...
  def load!
    path = Rails.root.join("config/shopify/metafield_schema.rb")
    ...
    Shop.with_shopify_session { load path }
  end
  ...
  def dump!
    schema_path = Rails.root.join("config/shopify/schema.rb")
    content = generate_schema_content
    File.write(schema_path, content)
    logger.info "Dumped metafield schema to #{schema_path}"
  end
end
